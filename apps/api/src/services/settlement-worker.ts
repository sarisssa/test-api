...
const computeFinalScores = async (
  ...
  await Promise.all(
    Array.from(tickers).map(async t => {
      const asset =
        await fastify.repositories.asset.fetchAssetByTickerFromDB(t);
...
const finalizeOneMatch = async (
  ...
  try {
    ...
    const updated = await fastify.repositories.match.completeMatchWithOutcome(
      match.matchId,
      {
    ...
    if (updated) {
      ...
      fastify.log.info({
        matchId: updated.matchId,
        winner,
        msg: 'Finalized time-expired match with winner',
      });
    }
  } catch (error) {
    if (
      ...
      fastify.log.info(
        ...
        'Settlement skipped: match mutated during finalize'
      );
      return;
    }
    fastify.log.error(
      { error, matchId: match.matchId },
...
export const startSettlementWorker = async (fastify: FastifyInstance) => {
  ...
  loop(fastify).catch(err =>
    fastify.log.error({ err }, 'Settlement worker crashed')
  );
};
