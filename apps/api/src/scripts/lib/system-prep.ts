...
export const DYNAMODB_ENDPOINT = process.env.DYNAMODB_URL ?? 'http://localhost:4566'
export const STEP_FUNCTIONS_ARN = process.env.MATCH_SETTLEMENT_STATE_MACHINE_ARN ?? ''
export const STEP_FUNCTIONS_ENDPOINT = process.env.STEP_FUNCTIONS_ENDPOINT
export const REDIS_URL = process.env.REDIS_URL ?? 'redis://127.0.0.1:6379'
export const REDIS_TLS_REJECT = process.env.REDIS_TLS_REJECT_UNAUTHORIZED ?? 'false'
...
export async function ensureStepFunctionsStateMachine(
  ...
  if (!stepFunctionsClient) {
    console.warn('⚠️  Step Functions client not configured; skipping state machine setup')
    return
  }
...
export async function seedStocks(tickers: string[] = STOCK_TICKERS): Promise<void> {
  await seedAssetPrices(
    tickers.map<AssetSeed>(symbol => ({ symbol, name: symbol, assetType: 'STOCK' }))
  )
...
async function seedAssetPrices(assets: AssetSeed[]): Promise<void> {
  ...
  const items = assets.map(({ symbol, assetType }) => ({
    PutRequest: {
      Item: {
        PK: `ASSET#${assetType}`,
        SK: symbol,
